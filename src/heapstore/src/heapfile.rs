...
impl HeapFile {
    ...
    pub fn num_pages(&self) -> PageId {
        let file_size = match self.file.read().unwrap().metadata() {
    ...
    pub(crate) fn read_page_from_file(&self, pid: PageId) -> Result<Page, CrustyError> {
        ...
        let mut page_size: [u8; PAGE_SIZE] = [0; PAGE_SIZE];
        match f.seek(SeekFrom::Start(pid as u64 * PAGE_SIZE as u64)) {
            Ok(_) => {}
            Err(_) => {
                return Err(CrustyError::CrustyError(
                    "Cannot find place to read from file".to_string(),
                ))
            }
        }
        match f.read_exact(&mut page_size) {
            Ok(_) => {}
            Err(_) => {
                return Err(CrustyError::CrustyError(
                    "Error with reading file".to_string(),
                ))
            }
        };
        let page: Page = Page::from_bytes(&page_size);
        Ok(page)
    }
    ...
    pub(crate) fn write_page_to_file(&self, page: Page) -> Result<(), CrustyError> {
        ...
        let mut f: std::sync::RwLockWriteGuard<File> = self.file.write().unwrap();
        let page_id: u16 = page.get_page_id();
        match f.seek(SeekFrom::Start(page_id as u64 * PAGE_SIZE as u64)) {
            Ok(_) => {}
            Err(_) => {
                return Err(CrustyError::CrustyError(
                    "Cannot find place to write to file".to_string(),
                ))
            }
        };
